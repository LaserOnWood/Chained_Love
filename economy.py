...
Commandes : /wallet, /shop add | list, /buy, /stats
"""
import discord
...
class ShopView(discord.ui.View):
    """Menu déroulant pour acheter un item."""
    ...
    async def select_callback(self, interaction: discord.Interaction):
        ...
        balance = await db.get_balance(interaction.client.pool, self.pair_id)

        if balance < item["cost"]:
            await interaction.response.send_message(
                embed=error(f"Solde insuffisant : {balance} pts / {item['cost']} pts requis."),
                ephemeral=True,
            )
            return

        purchase_id = await db.create_purchase(interaction.client.pool, item_id, self.pair_id)
        pair = await db.get_pair(interaction.client.pool, self.pair_id)

        view = PurchaseValidateView(
        ...
        notif = embed(
            "🛍️ Demande de récompense",
            f"<@{self.sub_id}> souhaite obtenir **{item['name']}** ({item['cost']} pts).",
            color="info",
        )
        try:
        ...
        await interaction.response.edit_message(
            embed=embed(
                ...
                + ("\n✉️ Ton/Ta Dominant·e a été notifié·e." if dm_ok else ""),
                color="ok",
            ),
            view=None,
        )
...
class PurchaseValidateView(discord.ui.View):
    """Boutons DOM pour valider/refuser un achat."""
    ...
    async def approve(self, interaction: discord.Interaction, button: discord.ui.Button):
        ...
        if interaction.user.id != pair["dom_id"]:
            ...
            return
        ok = await db.deduct_points(interaction.client.pool, self.pair_id, self.item["cost"])
        if not ok:
            await interaction.response.send_message(
                embed=error("Solde insuffisant au moment de la validation."), ephemeral=True
            )
            return
        await db.validate_purchase(interaction.client.pool, self.purchase_id, True)
        self.stop()
        await interaction.response.edit_message(
            embed=success(
                f"Récompense **{self.item['name']}** accordée à <@{self.sub_id}>.\n"
                f"—{self.item['cost']} pts déduits."
            ),
            view=None,
        )
    ...
    async def refuse(self, interaction: discord.Interaction, button: discord.ui.Button):
        ...
        if interaction.user.id != pair["dom_id"]:
            ...
            return
        await db.validate_purchase(interaction.client.pool, self.purchase_id, False)
        self.stop()
        await interaction.response.edit_message(
...
class Economy(commands.Cog):
    ...
    # ── /wallet ────────────────────────────────────────────────────────────────
    @app_commands.command(name="wallet", description="Affiche le solde de points d'une liaison.")
    ...
    async def shop_add(
        ...
        if not pair or pair["dom_id"] != interaction.user.id:
            ...
            return
        iid = await db.create_shop_item(self.bot.pool, pair["id"], name, description, cost)
        await interaction.response.send_message(
    ...
    async def shop_list(self, interaction: discord.Interaction, partner: discord.Member = None):
        ...
        items = await db.get_shop_items(self.bot.pool, pair["id"])
        if not items:
            await interaction.response.send_message(
    ...
    @app_commands.describe(partner="Ton/ta Dominant·e")
    async def buy(self, interaction: discord.Interaction, partner: discord.Member = None):
        ...
        if not items:
            await interaction.response.send_message(
                embed=embed("🛍️ Boutique vide", "Aucun item disponible pour le moment.", color="info"),
                ephemeral=True,
            )
            return
