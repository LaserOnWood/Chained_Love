...
Commandes : /safeword red | yellow, /limit add | remove | view
"""
import discord
...
class AftercareCancelView(discord.ui.View):
    ...
    async def resolve(self, interaction: discord.Interaction, button: discord.ui.Button):
        if interaction.user.id not in (self.dom_id, self.sub_id):
            ...
            return
        await db.resolve_safeword(self.event_id)
        await db.set_reminders(self.pair_id, True)
        self.stop()
        await interaction.response.edit_message(
            embed=success("✅ Safeword résolu. Les notifications reprennent normalement."), view=None
        )
...
class SafetyCog(commands.Cog, name="Safety"):
    ...
    async def safeword_red(self, interaction: discord.Interaction):
        ...
        if not pairs:
            ...
            return
        pair = pairs[0]
        event_id = await db.log_safeword(pair["id"], interaction.user.id, "RED")
        await db.set_reminders(pair["id"], False)
        ...
        try:
            ...
            await other.send(embed=e, view=AftercareCancelView(event_id, pair["id"], dom_id, sub_id))
        except Exception:
            pass
    ...
    async def safeword_yellow(self, interaction: discord.Interaction):
        ...
        if not pairs:
            ...
            return
        pair = pairs[0]
        event_id = await db.log_safeword(pair["id"], interaction.user.id, "YELLOW")
        await db.set_reminders(pair["id"], False)
        dom_id, sub_id = pair["dom_id"], pair["sub_id"]
        other_id = sub_id if interaction.user.id == dom_id else dom_id
        view = AftercareCancelView(event_id=event_id, pair_id=pair["id"], dom_id=dom_id, sub_id=sub_id)
        ...
        if not aftercare_ch:
            try:
                aftercare_ch = await guild.create_text_channel("aftercare", topic="Espace sécurisé. 🤍")
            except discord.Forbidden:
                pass
        if aftercare_ch:
            dom_m = guild.get_member(dom_id)
            sub_m = guild.get_member(sub_id)
            mentions = " ".join(m.mention for m in [dom_m, sub_m] if m)
            await aftercare_ch.send(content=mentions,
                                    embed=embed("🟡 Espace Aftercare",
    ...
    async def limit_add(self, interaction: discord.Interaction, label: str, color: str, notes: str = ""):
        ...
        if not pairs:
            ...
            return
        pair = pairs[0]
        lid = await db.add_limit(pair["id"], label, color, notes or None, uid)
        emoji_icon, clr, _ = LIMIT_COLORS[color]
        e = discord.Embed(title=f"{emoji_icon} Limite ajoutée",
                          description=f"**{label}**" + (f"\n_{notes}_" if notes else ""), color=clr)
    ...
    async def limit_remove(self, interaction: discord.Interaction, limit_id: int):
        ...
        row = await db.get_limit_by_id(limit_id)
        if not row or row["pair_id"] not in pair_ids:
            ...
            return
        await db.delete_limit(limit_id)
        await interaction.response.send_message(embed=warn(f"Limite **{row['name']}** supprimée."), ephemeral=True)
    ...
    async def limits_view(self, interaction: discord.Interaction, partner: discord.Member = None):
        ...
        if not pair:
            ...
            return
        limits = await db.get_limits(pair["id"])
        other_id = pair["sub_id"] if pair["dom_id"] == uid else pair["dom_id"]
        e = discord.Embed(title="📋 Registre des Limites",
