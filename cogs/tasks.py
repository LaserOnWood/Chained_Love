...
class ValidateView(discord.ui.View):
    ...
    async def approve(self, interaction: discord.Interaction, button: discord.ui.Button):
        ...
        if interaction.user.id != pair["dom_id"]:
            ...
            return
        await db.validate_completion(self.completion_id, True)
        await db.add_points(self.pair_id, self.task_points)
        self.stop()
    ...
    async def refuse(self, interaction: discord.Interaction, button: discord.ui.Button):
        ...
        if interaction.user.id != pair["dom_id"]:
            ...
            return
        await db.validate_completion(self.completion_id, False)
        self.stop()
        await interaction.response.edit_message(
...
class TasksCog(commands.Cog):
    ...
    @app_commands.describe(
        sub="Le/la subordonné·e concerné·e", name="Nom de la tâche",
        points="Points accordés à la validation", recurrence="Fréquence : daily | weekly | none",
        description="Description optionnelle", requires_proof="Preuve photo obligatoire ?",
    )
    async def task_add(self, interaction: discord.Interaction, sub: discord.Member, name: str,
                       points: int = 10, recurrence: str = "daily",
                       description: str = "", requires_proof: bool = False):
        ...
        if not pair or pair["dom_id"] != interaction.user.id:
            ...
            return
        tid = await db.create_task(pair["id"], name, description, points, recurrence, requires_proof)
        await interaction.response.send_message(
            embed=success(
    ...
    async def task_list(self, interaction: discord.Interaction, sub: discord.Member = None):
        ...
        if not pair:
            ...
            return
        task_list = await db.get_tasks(pair["id"])
    ...
    async def task_delete(self, interaction: discord.Interaction, task_id: int):
        ...
        if not task:
            ...
            return
        pair = await db.get_pair(task["pair_id"])
    ...
    async def done(self, interaction: discord.Interaction, task_id: int):
        ...
        if pair["sub_id"] != interaction.user.id:
            await interaction.response.send_message(embed=error("Cette tâche ne t'appartient pas."), ephemeral=True)
            return
        sw = await db.get_active_safeword(pair["id"])
        if sw:
            await interaction.response.send_message(
        ...
        if task["requires_proof"]:
            await interaction.response.send_message(
                embed=embed("📷 Preuve requise", "Envoie ta photo/vidéo dans ce canal dans les **2 minutes**.", color="warn")
            )
        ...
        completion_id = await db.add_completion(task_id, pair["id"], proof_url)
        view = ValidateView(completion_id=completion_id, pair_id=pair["id"],
                            task_points=task["points"], sub_id=interaction.user.id)
        notif_embed = embed(
    ...
    async def reminder_loop(self):
        ...
        for p in all_pairs:
            if p["checkin_hour"] != current_hour:
                continue
            try:
                task_list = await db.get_tasks(p["id"])
                if not task_list:
                    continue
